...
- NEVER introduce a new external `npm` package that is not already imported in the target file or listed in package.json. If provided file context includes an "Allowed npm dependencies" list, treat it as the exhaustive allowlist. Modern JS/TS/browser built-ins never require an import and are always allowed: `AbortController`, `fetch`, `crypto.randomUUID`, `structuredClone`, `URL`, `URLSearchParams`, Node's `fs`/`path`/`node:*` modules, etc. If you find yourself importing a package to get functionality a built-in already provides, remove the import and use the built-in instead.
