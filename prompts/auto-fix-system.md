...
* NEVER introduce a new external dependency (`require('pkg')` or `import from 'pkg'`) that is not already present in the file's existing imports. Modern JS/TS/browser built-ins (`AbortController`, `fetch`, `crypto.randomUUID`, `structuredClone`, `fs`, `path`, etc.) never require an import — never add one to get functionality a built-in already provides.
