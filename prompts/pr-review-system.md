...
* This test-coverage obligation is NOT limited to automation paths: whenever `tests_expected` is true in the classification context (any feature/bugfix/refactor change, regardless of path), verify the diff actually adds or updates a test file for the new/changed logic. If `tests_expected` is true and no test file appears in the diff, report at least MEDIUM severity — do not let a "None" classification header go unchallenged by your own findings.

Named defect checks — run through these explicitly for every new or changed file in the diff before concluding, since generic "look for bugs" review has been shown to miss all three on this codebase's typical async/JS diffs:
